...
c***** Integrating motion in separatrix****
     ...
     *T0,dn,n_tr,dn_jump,Etr
      external func
      external Voltage
      OPEN (UNIT=13,file='results/PS_SIN.dat',STATUS='unknown')
      OPEN (UNIT=14,file='results/IPS_SIN.dat',STATUS='unknown')
      OPEN (UNIT=15,file='results/PS_SIN_All.dat',STATUS='unknown')
...
c------solution of equations--------
     ...
     *E,eta,V,gamma,beta,eta0,eta1,dE_E,dp_p,turn)
      ...
      flag=1.0
      if((yy1.lt.(-T1-(T2/2)-100)).or.(yy1.gt.(T1+(T2/2)+100)))
     *flag=0.0
     ...
     *T0,dn,n_tr,dn_jump,Etr
      ...
      if(E.gt.Etr) sign = -1.0

      V = sign * Vbb * sin(2*3.14159*y(1)/(T0/2))
