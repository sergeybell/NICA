...
{
  TVector<T> z( x );
  auto multipy_by_c = [c](T x) { return x *c; };
  std::transform( z.begin(), z.end(), z.begin(), multipy_by_c);
  return z;
}
...
{
  auto multiply_by_c = [c](T x) { return x*c; };
 std::transform(begin(), end(), begin(), multiply_by_c); 
 return *this;
}
...
{
 TVector z( *this );
 auto divide_by_c = [c](T x) { return x/c; };
 std::transform( z.begin(), z.end(), z.begin(), divide_by_c); 
 return z;
}
...
{
  auto divide_by_c = [c](T x) { return x/c; };
  std::transform( begin(), end(), begin(), divide_by_c); 
  return *this;
}
